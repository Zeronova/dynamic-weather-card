...
\f\r]`,ka=`[^ 	
\f\r"'\`<>=]`,ya=`[^\\s"'>=/]`,D=/<(?:(!--|\/[^a-zA-Z])|(\/?[a-zA-Z][^>\s]*)|(\/?$))/g,ni=1,po=2,fa=3,li=/-->/g,si=/>/g,B=new RegExp(`>|${mo}(?:(${ya}+)(${mo}*=${mo}*(?:${ka}|("|')|))|$)`,"g"),wa=0,ri=1,ta=2,ci=3,bo=/'/g,Fo=/"/g,gi=/^(?:script|style|textarea|title)$/i,ma=1,lo=2,so=3,Ho=1,ro=2,pa=3,ba=4,Fa=5,Ao=6,Ma=7,Ko=(o)=>(i,...a)=>{if(i.some((n)=>n===void 0))console.warn(`Some template strings are undefined.
...
Please use the static 'html' tag function. See https://lit.dev/docs/templates/expressions/#static-expressions`);return{["_$litType$"]:o,strings:i,values:a}},k=Ko(ma),y=Ko(lo),ln=Ko(so),C=Symbol.for("lit-noChange"),p=Symbol.for("lit-nothing"),_i=new WeakMap,$=Q.createTreeWalker(Q,129),P=co;function ei(o,i){if(!zo(o)||!o.hasOwnProperty("raw")){let a="invalid template strings array";throw a=`
...
`,Va={sunny:y`
  ...
  `;hasContent(){if(!this.weather||!this.config)return!1;return this.config.showHumidity&&this.weather.humidity!=null||this.config.showWind&&this.weather.windSpeed!=null||this.config.showSunriseSunset&&this.sunData?.hasSunData===!0}renderHumidity(){if(!this.config?.showHumidity||this.weather?.humidity==null)return k``;return k`
    ...
    `}renderWind(){if(!this.config?.showWind||this.weather?.windSpeed==null)return k``;let o=this.entityAttributes||{},i=Jo(this.weather.windSpeed,o,this.config.windSpeedUnit),a=Qi(o,this.config.windSpeedUnit,d.t.bind(d)),n="";if(this.config.showWindGust&&this.weather.windGust)n=` / ${Jo(this.weather.windGust,o,this.config.windSpeedUnit)} ${a}`;let l=this.config.showWindDirection&&this.weather.windBearing!=null?Gi(this.weather.windBearing):E("wind");return k`
      <div class="info-item">
    ...
    `}}w([t({type:Object})],ji.prototype,"weather",void 0),w([t({type:Object})],ji.prototype,"sunData",void 0),w([t({type:Object})],ji.prototype,"config",void 0),w([t({type:Object})],ji.prototype,"entityAttributes",void 0);customElements.define("weather-details",ji);var uo=W`
  :host {
    display: block;
  }
