...
Please use the static 'html' tag function. See https://lit.dev/docs/templates/expressions/#static-expressions`);return{["_$litType$"]:o,strings:i,values:a}},u=Ao(za),k=Ao(lo),cn=Ao(so),W=Symbol.for("lit-noChange"),p=Symbol.for("lit-nothing"),ci=new WeakMap,q=J.createTreeWalker(J,129),$=co;function ti(o,i){if(!So(o)||!o.hasOwnProperty("raw")){let a="invalid template strings array";throw a=`
...
`,Ua={sunny:k`
  ...
  `};function E(o,...i){let a=Xa[o];if(typeof a==="function")return a(...i);return a||""}function to(o){if(!o)return"";return Ua[o.toLowerCase()]||""}class Vi extends z{constructor(){super(...arguments);this.weather=null;this.sunData=null;this.config=null;this.entityAttributes=null;this.fontSize=13}static styles=Q`
    :host {
      display: block;
      --dw-font-size: 13px;
    }
  ...
  `;hasContent(){if(!this.weather||!this.config)return!1;return this.config.showHumidity&&this.weather.humidity!=null||this.config.showWind&&this.weather.windSpeed!=null||this.config.showSunriseSunset&&this.sunData?.hasSunData===!0}renderHumidity(){if(!this.config?.showHumidity||this.weather?.humidity==null)return u``;return u`
    ...
    `}renderSunrise(){if(!this.config?.showSunriseSunset||!this.sunData?.hasSunData||!this.sunData.sunrise)return u``;return u`
      <div class="info-item">
        ...
        <span>${Zo(this.sunData.sunrise,this.config.clockFormat,e.t("am"),e.t("pm"))}</span>
      </div>
    `}renderWind(){if(!this.config?.showWind||this.weather?.windSpeed==null)return u``;let o=this.entityAttributes||{},i=xo(this.weather.windSpeed,o,this.config.windSpeedUnit),a=Pi(o,this.config.windSpeedUnit,e.t.bind(e)),n="";if(this.config.showWindGust&&this.weather.windGust)n=` / ${xo(this.weather.windGust,o,this.config.windSpeedUnit)} ${a}`;let l=this.config.showWindDirection&&this.weather.windBearing!=null?Gi(this.weather.windBearing):E("wind");return u`
      <div class="info-item">
    ...
    `}renderSunset(){if(!this.config?.showSunriseSunset||!this.sunData?.hasSunData||!this.sunData.sunset)return u``;return u`
      <div class="info-item">
        ...
        <span>${Zo(this.sunData.sunset,this.config.clockFormat,e.t("am"),e.t("pm"))}</span>
      </div>
    ...
    `}}w([f({type:Object})],Vi.prototype,"weather",void 0),w([f({type:Object})],Vi.prototype,"sunData",void 0),w([f({type:Object})],Vi.prototype,"config",void 0),w([f({type:Object})],Vi.prototype,"entityAttributes",void 0),w([f({type:Number})],Vi.prototype,"fontSize",void 0);customElements.define("weather-details",Vi);var go=Q`
  :host {
    display: block;
  }
...
Динамическая карточка погоды`),window.customCards=window.customCards||[];let o={type:"dynamic-weather-card",name:"Dynamic Weather Card",description:"Динамическая карточка погоды",preview:!0,documentationURL:"https://github.com/teuchezh/dynamic-weather-card"};window.customCards.push(o)}catch(o){console.error("❌ Ошибка при регистрации Dynamic Weather Card:",o)}export{on as t,I as resolveLanguage,e as i18n};
