...
Please use the static 'html' tag function. See https://lit.dev/docs/templates/expressions/#static-expressions`);return{["_$litType$"]:o,strings:i,values:a}},d=Fo(ba),y=Fo(lo),cn=Fo(so),$=Symbol.for("lit-noChange"),p=Symbol.for("lit-nothing"),ci=new WeakMap,W=C.createTreeWalker(C,129),Q=co;function ti(o,i){if(!So(o)||!o.hasOwnProperty("raw")){let a="invalid template strings array";throw a=`
...
`,Ra={sunny:y`
  ...
  `};function q(o,...i){let a=Xa[o];if(typeof a==="function")return a(...i);return a||""}function to(o){if(!o)return"";return Ra[o.toLowerCase()]||""}class ji extends b{constructor(){super(...arguments);this.weather=null;this.sunData=null;this.config=null;this.entityAttributes=null;this.fontSize=13;this.hass=null}static styles=P`
    :host {
      display: block;
      --dw-font-size: 13px;
    }
  ...
  `;hasContent(){if(!this.weather||!this.config)return!1;return this.config.showHumidity&&this.weather.humidity!=null||this.hasPrecipitation()||this.config.showWind&&this.weather.windSpeed!=null||this.config.showWindDirection&&this.weather.windBearing!=null||this.config.showSunrise&&this.sunData?.sunrise!=null||this.config.showSunset&&this.sunData?.sunset!=null||!!this.config.detailEntity&&!!this.hass?.states[this.config.detailEntity]||!!this.config.showMoon&&!!this.config.moonEntity&&!!this.hass?.states[this.config.moonEntity]}getPrecipitationValue(){if(this.config?.precipitationEntity&&this.hass){let o=this.hass.states[this.config.precipitationEntity];if(o&&o.state!=="unavailable"&&o.state!=="unknown"){let i=parseFloat(o.state);if(!isNaN(i))return i}}return this.weather?.precipitation??null}getEntityState(o){let i=this.hass?.states?.[o];if(!i||i.state==="unavailable"||i.state==="unknown")return null;return i.state}getPrecipitationUnit(){if(this.config?.precipitationEntity&&this.hass){let i=this.hass.states[this.config.precipitationEntity];if(i?.attributes?.unit_of_measurement)return i.attributes.unit_of_measurement}return(this.entityAttributes||{}).precipitation_unit||"mm"}hasPrecipitation(){if(this.config?.precipitationEntity&&this.hass){let o=this.hass.states[this.config.precipitationEntity];return!!(o&&o.state!=="unavailable"&&o.state!=="unknown")}return this.weather?.precipitation!=null}renderHumidity(){if(!this.config?.showHumidity||this.weather?.humidity==null)return d``;return d`
    ...
    `}renderWind(){if(!this.config?.showWind||this.weather?.windSpeed==null)return d``;let o=this.entityAttributes||{},i=vo(this.weather.windSpeed,o,this.config.windSpeedUnit),a=Pi(o,this.config.windSpeedUnit,e.t.bind(e)),n="";if(this.config.showWindGust&&this.weather.windGust)n=` / ${vo(this.weather.windGust,o,this.config.windSpeedUnit)} ${a}`;return d`
      <div class="info-item">
        <span class="info-icon">${q("wind")}</span>
        <span>${i} ${a}${n}</span>
      </div>
    ...
    `}renderIcon(o,i){let a=o||i||"";if(!a)return q("humidity");if(a.startsWith("mdi:"))return d`<ha-icon icon="${a}"></ha-icon>`;let n=q(a);if(typeof n!=="string")return n;return q("humidity")}renderDetailEntity(){if(!this.config?.detailEntity||!this.hass)return d``;let o=this.hass.states[this.config.detailEntity];if(!o||o.state==="unavailable"||o.state==="unknown")return d``;let i=o.attributes?.friendly_name||this.config.detailEntity,a=o.state,n=o.attributes?.unit_of_measurement||"",l=o.attributes?.icon||"";return d`
    ...
    `}renderMoonPhase(){if(!this.config?.showMoon||!this.config?.moonEntity||!this.hass)return d``;let o=this.hass.states[this.config.moonEntity];if(!o||o.state==="unavailable"||o.state==="unknown")return d``;let i=o.state,a=`mdi:moon-${i.replace(/_/g,"-").replace(/-moon$/,"")}`,n=o.attributes?.friendly_name||"Mond",l=i.replace(/_/g," ");return d`
      <div class="info-item detail-with-name">
        <span class="info-icon"><ha-icon icon="${a}"></ha-icon></span>
        <span class="detail-text">
          <span class="detail-name">${n}</span>
...
`;class Yi extends b{constructor(){super(...arguments);this.forecast=[];this.clockFormat="24h";this.layout="horizontal";this.compact=!1}static styles=ho;_cleanup=null;connectedCallback(){super.connectedCallback(),this.updateComplete.then(()=>{if(this.layout==="horizontal")this._cleanup=eo(this.shadowRoot,".forecast-scroll")})}disconnectedCallback(){super.disconnectedCallback(),this._cleanup?.(),this._cleanup=null}getTemperature(o){return Math.round(o.temperature??o.temp??o.native_temperature??0)}render(){if(this.forecast.length===0)return d``;return d`
        ...
        <div class="forecast-scroll${this.layout==="vertical"?" forecast-scroll--vertical":""}">
          ${this.forecast.map((o)=>d`
            <div class="forecast-item">
              ...
              <div class="forecast-icon">${to(o.condition||"sunny")}</div>
              <div class="forecast-temp">${this.getTemperature(o)}°</div>
            </div>
          `)}
        </div>
      </div>
    `}}f([w({type:Array})],Yi.prototype,"forecast",void 0),f([w({type:String})],Yi.prototype,"clockFormat",void 0),f([w({type:String})],Yi.prototype,"layout",void 0),f([w({type:Boolean})],Yi.prototype,"compact",void 0);customElements.define("hourly-forecast",Yi);class Xi extends b{constructor(){super(...arguments);this.forecast=[];this.lang="en";this.layout="horizontal";this.compact=!1}static styles=ho;_cleanup=null;connectedCallback(){super.connectedCallback(),this.updateComplete.then(()=>{if(this.layout==="horizontal")this._cleanup=eo(this.shadowRoot,".forecast-scroll")})}disconnectedCallback(){super.disconnectedCallback(),this._cleanup?.(),this._cleanup=null}getTemperature(o){return Math.round(o.temperature??o.temp??o.native_temperature??0)}render(){if(this.forecast.length===0)return d``;return d`
      <div class="forecast-container${this.compact?" forecast-container--compact":""}">
    ...
    `}getCustomEntityStates(){if(!this.hass||!this.config.customEntities?.length)return[];return this.config.customEntities.slice(0,3).map((o)=>{let i=this.hass.states[o.entity];if(!i)return null;return{state:i.state,name:o.name||i.attributes.friendly_name||o.entity}}).filter((o)=>o!==null)}setConfig(o){if(!o.entity)throw Error("Please define a weather entity");let i=o.show_hourly_forecast??o.show_forecast;if(this.config={type:"custom:dynamic-weather-card",entity:o.entity,icons_path:o.icons_path,name:o.name,height:o.height||t.height,showFeelsLike:o.show_feels_like!==!1,showWind:o.show_wind!==!1,showWindGust:o.show_wind_gust!==!1,showWindDirection:o.show_wind_direction!==!1,showHumidity:o.show_humidity!==!1,showPrecipitation:o.show_precipitation===!0,showSunrise:o.show_sunrise??o.show_sunrise_sunset??!0,showSunset:o.show_sunset??o.show_sunrise_sunset??!0,showMinTemp:o.show_min_temp!==!1,showForecast:o.show_forecast===!0,showHourlyForecast:i===!0,showDailyForecast:o.show_daily_forecast===!0,hourlyForecastHours:o.hourly_forecast_hours??t.hourlyForecastHours,dailyForecastDays:o.daily_forecast_days??t.dailyForecastDays,showSunriseSunset:o.show_sunrise_sunset!==!1,showAnimation:o.show_animation!==!1,showClock:o.show_clock===!0,clockPosition:o.clock_position||t.clockPosition,clockFormat:o.clock_format||t.clockFormat,clockSize:o.clock_size??t.clockSize,showDate:o.show_date===!0,showSeconds:o.show_seconds??t.showSeconds,forecastLayout:o.forecast_layout||t.forecastLayout,overlayOpacity:o.overlay_opacity!==void 0?o.overlay_opacity:t.overlayOpacity,language:o.language||t.language,windSpeedUnit:o.wind_speed_unit||t.windSpeedUnit,sunriseEntity:o.sunrise_entity||null,sunsetEntity:o.sunset_entity||null,templowAttribute:o.templow_attribute||null,detailEntity:o.detail_entity||"",detailEntityIcon:o.detail_entity_icon||o.detailEntityIcon||"",showMoon:o.show_moon!==!1,moonEntity:o.moon_entity||t.moonEntity,precipitationEntity:o.precipitation_entity||"",tapAction:o.tap_action||{action:"more-info"},holdAction:o.hold_action||{action:"none"},doubleTapAction:o.double_tap_action||{action:"none"},timeBackground:o.time_background||void 0,fontSize:o.font_size??t.fontSize,customEntities:o.custom_entities?.length?o.custom_entities.slice(0,3).map((a)=>({entity:typeof a==="string"?a:a.entity,name:typeof a==="string"?void 0:a.name})):[]},this.config.language)e.setLanguage(this.config.language)}fireEvent(o,i={}){let a=new CustomEvent(o,{detail:i,bubbles:!0,composed:!0});this.dispatchEvent(a)}getCardSize(){return 1}render(){if(!this.hass)return d`<div>No Home Assistant connection</div>`;let o=Go(this.hass,this.config.entity,this.config,this.forecastService.getHourlyData()),i=this.hass.states[this.config.entity],a=xo(i,this.config.sunriseEntity,this.config.sunsetEntity,this.hass),n=this._testTimeOfDay||No(a),l=`weather-card ${n.type}`,s=this.config.height?`${this.config.height}px`:"200px",r=this.config.timeBackground?.length?Ci(this.config.timeBackground):Wi(n),c=r?`background: linear-gradient(135deg, rgb(${r.start.r}, ${r.start.g}, ${r.start.b}), rgb(${r.end.r}, ${r.end.g}, ${r.end.b}));`:"",g=`--overlay-opacity: ${this.config.overlayOpacity!==void 0?this.config.overlayOpacity:t.overlayOpacity};`,h=this.config.showHourlyForecast?this.forecastService.getHourlyForecast(this.config.hourlyForecastHours??t.hourlyForecastHours,o):[],k=this.config.showDailyForecast?this.forecastService.getDailyForecast(this.config.dailyForecastDays??t.dailyForecastDays,o):[];return d`
      <ha-card
    ...
    `}}f([w({type:Object})],Vo.prototype,"hass",void 0),f([w({type:Object})],Vo.prototype,"config",void 0);class jo extends b{constructor(){super(...arguments);this._config={}}setConfig(o){let i={},a=o.custom_entities||o.customEntities||[];if(Array.isArray(a))a.forEach((s,r)=>{if(r<3&&s?.entity){if(i[`custom_entity_${r+1}`]=s.entity,s.name)i[`custom_entity_${r+1}_name`]=s.name}});let n=o;if(!n.forecast_layout&&n.forecastLayout)i.forecast_layout=n.forecastLayout;let l=o.detail_entity_icon||o.detailEntityIcon;if(l)i.detail_entity_icon=l;this._config={name:"",height:t.height,show_feels_like:t.showFeelsLike,show_wind:t.showWind,show_wind_gust:t.showWindGust,show_wind_direction:t.showWindDirection,show_humidity:t.showHumidity,show_precipitation:t.showPrecipitation,show_sunrise:t.showSunrise,show_sunset:t.showSunset,show_min_temp:t.showMinTemp,show_hourly_forecast:t.showHourlyForecast,hourly_forecast_hours:t.hourlyForecastHours,show_daily_forecast:t.showDailyForecast,daily_forecast_days:t.dailyForecastDays,show_animation:t.showAnimation,show_clock:t.showClock,clock_position:t.clockPosition,clock_format:t.clockFormat,clock_size:t.clockSize,show_date:t.showDate,show_seconds:t.showSeconds,forecast_layout:t.forecastLayout,overlay_opacity:t.overlayOpacity,language:t.language,wind_speed_unit:t.windSpeedUnit,sunrise_entity:"",sunset_entity:"",precipitation_entity:"",custom_entity_1:"",custom_entity_1_name:"",custom_entity_2:"",custom_entity_2_name:"",custom_entity_3:"",custom_entity_3_name:"",detail_entity:"",detail_entity_icon:"",show_moon:t.showMoon,moon_entity:t.moonEntity,...o,...i}}updated(o){if(super.updated(o),o.has("hass")){let i=E({hassLang:this.hass?.language});if(e.lang!==i)e.setLanguage(i),this.requestUpdate()}}get _schema(){return[{name:"entity",required:!0,selector:{entity:{domain:["weather"]}}},{name:"name",selector:{text:{}}},{name:"height",selector:{number:{min:200,max:800,step:10,mode:"box"}}},{name:"show_feels_like",selector:{boolean:{}}},{name:"show_wind",selector:{boolean:{}}},{name:"show_wind_gust",selector:{boolean:{}}},{name:"show_wind_direction",selector:{boolean:{}}},{name:"show_humidity",selector:{boolean:{}}},{name:"show_precipitation",selector:{boolean:{}}},{name:"precipitation_entity",selector:{entity:{}}},{name:"show_sunrise",selector:{boolean:{}}},{name:"show_sunset",selector:{boolean:{}}},{name:"show_min_temp",selector:{boolean:{}}},{name:"show_hourly_forecast",selector:{boolean:{}}},{name:"hourly_forecast_hours",selector:{number:{min:1,max:24,step:1,mode:"box"}}},{name:"show_daily_forecast",selector:{boolean:{}}},{name:"daily_forecast_days",selector:{number:{min:1,max:14,step:1,mode:"box"}}},{name:"sunrise_entity",selector:{entity:{}}},{name:"sunset_entity",selector:{entity:{}}},{name:"show_clock",selector:{boolean:{}}},{name:"clock_position",selector:{select:{options:[{label:e.t("editor.clock_position_top"),value:"top"},{label:e.t("editor.clock_position_details"),value:"details"}]}}},{name:"clock_format",selector:{select:{options:[{label:e.t("editor.clock_format_24h"),value:"24h"},{label:e.t("editor.clock_format_12h"),value:"12h"}]}}},{name:"clock_size",selector:{number:{min:16,max:120,step:2,mode:"box"}}},{name:"show_date",selector:{boolean:{}}},{name:"show_seconds",selector:{boolean:{}}},{name:"forecast_layout",selector:{select:{options:[{label:e.t("editor.forecast_layout_horizontal"),value:"horizontal"},{label:e.t("editor.forecast_layout_vertical"),value:"vertical"},{label:e.t("editor.forecast_layout_original"),value:"original"}]}}},{name:"overlay_opacity",selector:{number:{min:0,max:1,step:0.05,mode:"box"}}},{name:"font_size",selector:{number:{min:8,max:30,step:1,mode:"box"}}},{name:"custom_entity_1",selector:{entity:{domain:"sensor",device_class:["temperature"]}}},{name:"custom_entity_1_name",selector:{text:{}}},{name:"custom_entity_2",selector:{entity:{domain:"sensor",device_class:["temperature"]}}},{name:"custom_entity_2_name",selector:{text:{}}},{name:"custom_entity_3",selector:{entity:{domain:"sensor",device_class:["temperature"]}}},{name:"custom_entity_3_name",selector:{text:{}}},{name:"detail_entity",selector:{entity:{}}},{name:"detail_entity_icon",selector:{icon_picker:{}}},{name:"show_moon",selector:{boolean:{}}},{name:"moon_entity",selector:{entity:{domain:"sensor"}}},{name:"language",selector:{select:{options:[{label:e.t("editor.language_auto"),value:"auto"},{label:e.t("editor.language_en"),value:"en"},{label:e.t("editor.language_ru"),value:"ru"},{label:e.t("editor.language_de"),value:"de"},{label:e.t("editor.language_nl"),value:"nl"},{label:e.t("editor.language_fr"),value:"fr"},{label:e.t("editor.language_es"),value:"es"},{label:e.t("editor.language_it"),value:"it"},{label:e.t("editor.language_sk"),value:"sk"},{label:e.t("editor.language_hu"),value:"hu"}]}}},{name:"wind_speed_unit",selector:{select:{options:[{label:e.t("editor.wind_speed_unit_ms"),value:"ms"},{label:e.t("editor.wind_speed_unit_kmh"),value:"kmh"}]}}},{name:"show_animation",selector:{boolean:{}}}]}_computeLabel=(o)=>{let i=`editor.${o.name}`,a=e.t(i);return a===i?o.name:a};_valueChanged(o){let i=o.detail?.value;if(!i)return;let a=[];for(let n=1;n<=3;n++){let l=`custom_entity_${n}`,s=`custom_entity_${n}_name`;if(i[l])a.push({entity:i[l],name:i[s]||void 0});delete i[l],delete i[s]}if(a.length)i.custom_entities=a;if(i.detail_entity_icon)i.detail_entity_icon=i.detail_entity_icon;this._config=i,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:this._config},bubbles:!0,composed:!0}))}render(){if(!this.hass)return d``;return d`
      <ha-form
        .hass=${this.hass}
        .data=${this._config}
        .schema=${this._schema}
      ...
      ></ha-form>
    `}}f([w({attribute:!1})],jo.prototype,"hass",void 0),f([pi()],jo.prototype,"_config",void 0);var Ri={ATTRIBUTE:1,CHILD:2,PROPERTY:3,BOOLEAN_ATTRIBUTE:4,EVENT:5,ELEMENT:6},Ui=(o)=>(...i)=>({["_$litDirective$"]:o,values:i});class Yo{constructor(o){}get _$isConnected(){return this._$parent._$isConnected}_$initialize(o,i,a){this.__part=o,this._$parent=i,this.__attributeIndex=a}_$resolve(o,i){return this.update(o,i)}update(o,i){return this.render(...i)}}var Ua=!0,sr=Ua&&window.ShadyDOM?.inUse&&window.ShadyDOM?.noPatch===!0?window.ShadyDOM.wrap:(o)=>o;var Li=(o)=>o.strings===void 0;var La=!0,oo=(o,i)=>{let a=o._$disconnectableChildren;if(a===void 0)return!1;for(let n of a)n._$notifyDirectiveConnectionChanged?.(i,!1),oo(n,i);return!0},go=(o)=>{let i,a;do{if((i=o._$parent)===void 0)break;a=i._$disconnectableChildren,a.delete(o),o=i}while(a?.size===0)},Ii=(o)=>{for(let i;i=o._$parent;o=i){let a=i._$disconnectableChildren;if(a===void 0)i._$disconnectableChildren=a=new Set;else if(a.has(o))break;a.add(o),Oa(i)}};function Ia(o){if(this._$disconnectableChildren!==void 0)go(this),this._$parent=o,Ii(this);else this._$parent=o}function Ea(o,i=!1,a=0){let n=this._$committedValue,l=this._$disconnectableChildren;if(l===void 0||l.size===0)return;if(i){if(Array.isArray(n))for(let s=a;s<n.length;s++)oo(n[s],!1),go(n[s]);else if(n!=null)oo(n,!1),go(n)}else oo(this,o)}var Oa=(o)=>{if(o.type==Ri.CHILD)o._$notifyConnectionChanged??=Ea,o._$reparentDisconnectables??=Ia};class Xo extends Yo{constructor(){super(...arguments);this._$disconnectableChildren=void 0}_$initialize(o,i,a){super._$initialize(o,i,a),Ii(this),this.isConnected=o._$isConnected}["_$notifyDirectiveConnectionChanged"](o,i=!0){if(o!==this.isConnected)if(this.isConnected=o,o)this.reconnected?.();else this.disconnected?.();if(i)oo(this,o),go(this)}setValue(o){if(Li(this.__part))this.__part._$setValue(o,this);else{if(La&&this.__attributeIndex===void 0)throw Error("Expected this.__attributeIndex to be a number");let i=[...this.__part._$committedValue];i[this.__attributeIndex]=o,this.__part._$setValue(i,this,0)}}disconnected(){}reconnected(){}}class Ei extends Xo{_key="";_onLangChange=null;render(o){return this._key=o,e.t(o)}reconnected(){this._onLangChange=()=>{this.setValue(e.t(this._key))},window.addEventListener("language-changed",this._onLangChange)}disconnected(){if(this._onLangChange)window.removeEventListener("language-changed",this._onLangChange)}}var on=Ui(Ei);try{customElements.define("dynamic-weather-card",Vo),customElements.define("dynamic-weather-card-editor",jo),console.log(`%cDynamic Weather Card %c${zi}`,"color: #007AFF; font-weight: bold; font-size: 14px;","color: #666; font-size: 12px;",`
